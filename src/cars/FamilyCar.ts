...
export function FamilyCar(): THREE.Object3D {
  const car = new THREE.Group();

  // Corpo de passeio
  const bodyGeometry = new THREE.BoxGeometry(2.2, 0.7, 4.2);
  ...
  // Janelas
  const windowGeometry = new THREE.BoxGeometry(1.8, 0.4, 2.2);
  ...
  for (let i = -1; i <= 1; i += 2) {
    for (let j = -1; j <= 1; j += 2) {
      ...
      wheel.position.set(i * 0.95, 0.2, j * 1.5);
      car.add(wheel);
    }
  }
